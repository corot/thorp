...
void BioloidController::readPose()
{
        ...
        for(iter=0; iter<numServos_; iter++)
        {
                unsigned int temp = pDxl->readWord(id_[iter], AXM_PRESENT_POSITION_L);
...
void BioloidController::writePose()
{
        ...
        pDxl->setTxPacketParameter( 0, AXM_GOAL_POSITION_L );
...
{
        bcState_ = KILLED;
        pDxl->writeByte(BROADCAST_ID, AXM_TORQUE_ENABLE, 0);
}
