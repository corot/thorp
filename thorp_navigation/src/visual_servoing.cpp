...
VS::VS(int argc, char**argv)
{
  ...
  subStatus_ = pnh_.subscribe("status", 1, &VS::statusCallback, this);
  subPose_   = nh_.subscribe("target_object_pose", 1, &VS::poseCallback, this);
  pubTwist_  = nh_.advertise<geometry_msgs::Twist>("cmd_vel", 1);
  ...
  sub_cam_info = nh_.subscribe("camera_info", 1, &VS::CameraInfoCb,this);
