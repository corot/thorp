                                          ...
                                          const float gripper_tightening_closing)
{
  ...
  double abs_diff_ = std::abs(ttk::minAngle(grasp_pose, obj_pose));     // absolute angles difference
  double abs_diff = abs_diff_ > M_PI_2 ? M_PI - abs_diff_ : abs_diff_;  // normalized to 0 .. pi/2
  double diag_angle = std::atan(obj_size.y / obj_size.x);  // angle of the diagonal of the obj base
  bool x_aligned = abs_diff + diag_angle < M_PI_2;         // combined decided the side we will grasp
  double closing = x_aligned ? obj_size.y : obj_size.x;
  ROS_DEBUG("%f\t%f\t%f\t%f\t%f\t%f\t%d\t%f\t%f", heading, obj_yaw, abs_diff_, abs_diff, diag_angle, abs_diff + diag_angle, x_aligned, obj_size.x, obj_size.y);
  ROS_DEBUG_COND(x_aligned, "using Y");
  ROS_DEBUG_COND(!x_aligned, "using X");
