...
std::vector<Object> SpatialHash::getObjectsInRegion(const Point2d& lower_left, double size_x, double size_y) const
{
  ...
  for (auto bucket_id : hash_ids)
  {
    ...
    catch (std::out_of_range& e) { }  // this is fine; means this hash has been never used; but I wonder if this has any performance penalization...
  }
...
//     }
//  }
//  ROS_INFO_STREAM("In spatialHash::getAllCellsOccupiedByPolygon: bounding box tl is: "<<obstacle.bounding_box.tl()<<" bounding box br is: "<<obstacle.bounding_box.br());
...
//      ROS_WARN("In getAllCellsOccupiedByPolygon (spatial hash), got an empty obstacle with id: %d, it has: %d points",obstacle.id, obstacle.contour_points.size());
//      ROS_INFO_STREAM("Cellsize is: "<<cell_size_);
//      ROS_INFO_STREAM("Loop bounds for x would have been: "<<std::floor(min_x/cell_size_)*cell_size_<<" and "<<std::ceil(max_x/cell_size_)*cell_size_);
//      ROS_INFO_STREAM("Loop bounds for y would have been: "<<std::floor(min_y/cell_size_)*cell_size_<<" and "<<std::ceil(max_y/cell_size_)*cell_size_);
//      ROS_INFO("My points are: ");
//      for (auto point: obstacle.contour_points) {
...
//      min_y = min_y-0.1;
//      max_y = max_y=0.1;
//      ROS_INFO_STREAM("Loop bounds for x now are: "<<std::floor(min_x/cell_size_)*cell_size_<<" and "<<std::ceil(max_x/cell_size_)*cell_size_);
...
//    }
  ...
  // line object can have no extent along one of the axis possibly (problem with line objects remaining stuck within the hash)
  if (std::floor(min_x/cell_size_) == std::ceil(max_x/cell_size_))
  {
//    for (float y =std::floor(min_y/cell_size_)*cell_size_; y<std::ceil(max_y/cell_size_)*cell_size_; y+=cell_size_) {
...
//  Rectangle bucket_rect;
  ...
  for (float x = std::floor(min_x / cell_size_) * cell_size_;
             x < std::ceil(max_x / cell_size_) * cell_size_; x += cell_size_)
  {
    for (float y = std::floor(min_y / cell_size_) * cell_size_;
...
//      }
      ...
      if (do_visualization) {
        ...
        for (auto point : object.contour_points) {
          contour_points[0].push_back(cv::Point(point.x/cell_size_*cell_resolution, point.y/cell_size_*cell_resolution));
        }
...
void SpatialHash::updateStoredObject(const Object& object)
{
  ...
  objects_[object.id].contour_center = object.contour_center;
