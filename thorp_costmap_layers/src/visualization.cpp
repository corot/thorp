...
Visualization::Visualization(ros::NodeHandle& pnh)
{
  ...
  if (pnh.param<bool>("show_debug_markers", false))
    debug_markers_pub_ = pnh.advertise<visualization_msgs::Marker>("visual_markers", 1);
}
                                      ...
                                      const std::string& frame_id)
{
  ...
  showLineStrip({Point2d(new_min_x, new_min_y),
                 Point2d(new_min_x, new_max_y),
                 Point2d(new_max_x, new_max_y),
                 Point2d(new_max_x, new_min_y),
                 Point2d(new_min_x, new_min_y)}, frame_id, 1, "updated bounds", "white");
  showLineStrip({Point2d(old_min_x, old_min_y),
                 Point2d(old_min_x, old_max_y),
                 Point2d(old_max_x, old_max_y),
                 Point2d(old_max_x, old_min_y),
                 Point2d(old_min_x, old_min_y)}, frame_id, 1, "previous bounds", "beige", 0.5f);
}
...
void Visualization::showLineStrip(const std::list<Point2d>& points, const std::string& frame_id,
                                  int id, const std::string& ns, const std::string& color, float alpha)
{
  if (!debug_markers_pub_.getNumSubscribers())
    return;
